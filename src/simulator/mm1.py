"""
M/M/1 queue simulation module.

This module implements the M/M/1 queueing system, which is a single-server queue
...
class QueueEvent(Enum):
    """Enumeration of possible events in a queueing system."""
    ARRIVAL = "arrival"  # Customer arrives to the system
    DEPARTURE = "departure"  # Customer completes service and departs
    SERVICE_RATE_CHANGE = "service_rate_change"  # Service rate changes (for control)

class MM1Queue(CTMCSimulator):
    """
    Simulator for an M/M/1 queueing system.
    ...
    - The state is the number of customers in the system (including the one in service)
    """
    ...
    def __init__(
        ...
        """
        Initialize an M/M/1 queue simulator.
    ...
    def get_transition_rates(self) -> Dict[Tuple[QueueEvent, int], float]:
        ...
        if queue_length > 0:
            transitions[(QueueEvent.DEPARTURE, queue_length - 1)] = self.service_rate
    ...
    def set_service_rate(self, new_rate: float):
        """
        Change the service rate of the queue.
        
        This method can be used for dynamic control of the queue.
        
        Args:
            new_rate: New service rate (must be positive)
        """
        if new_rate <= 0:
            raise ValueError("Service rate must be positive")
    ...
    def get_utilization(self) -> float:
        ...
        Returns:
            The ratio λ/μ or 1.0 if the system is overloaded
        """
